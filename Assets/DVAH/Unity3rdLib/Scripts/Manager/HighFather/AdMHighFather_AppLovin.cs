...
{ 
    public class AdMHighFather_AppLovin : AdMHighFather
    {
        ...
        {
            // Banner ad failed to load. MAX will automatically try loading ReplayCount new ad internally.
        ...
        {
            // Interstitial ad failed to load. We recommend retrying with exponentially higher delays up to ReplayCount maximum delay (in this case 64 seconds).
            interstitialRetryAttempt++;
        ...
        {
            // Rewarded ad failed to load. We recommend retrying with exponentially higher delays up to ReplayCount maximum delay (in this case 64 seconds).
        ...
        public override void ShowInterstitial(Action<InterVideoState> callback = null, bool showNoAds = false)
        {
            if (_offAdPosition[(int)AD_TYPE.inter])
            {
                ...
                Debug.Log("ShowInterstitial - adm 1 ");
                return;
            }
            Debug.Log("ShowInterstitial - InterstitialIsLoaded " + InterstitialIsLoaded());
            ...
            try
            {
                Debug.Log("ShowInterstitial - try ");
                callback?.Invoke(InterVideoState.None);
            }
            catch (Exception e)
            {
